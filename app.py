...
# ── Symbol → Kite instrument token lookup (top 50 pre-cached) ─
# Full list: call kite.instruments("NSE") once and save
_KITE_TOKENS = {
...
def _get_data_kite(ticker_ns: str, interval: str, period: str):
    """Fetch OHLCV from Zerodha Kite historical API."""
    kite = st.session_state.get("kite")
    if not kite: return None
    sym  = ticker_ns.replace(".NS","")
    token= _KITE_TOKENS.get(sym)
