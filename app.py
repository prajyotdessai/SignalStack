NSE PRO TRADER v5.2 — BUG FIXES (Scan results now visible)
============================================================
BUGS FIXED vs v5.1:
  🐛 BUG 1 — @st.cache_data cannot cache KiteConnect objects.
             _fetch_instrument_tokens() was decorated with @st.cache_data
             but KiteConnect is not serialisable → tokens silently never
             loaded → every symbol got token=None → all data_cache=None
             → zero results.
             FIX: Removed @st.cache_data; tokens cached in session_state
             with a manual "loaded" flag.

  🐛 BUG 2 — get_data_kite() called _fetch_instrument_tokens() internally
             (another fresh KiteConnect per call). Inside a parallel
             ThreadPoolExecutor this caused 16 simultaneous token-fetches
             — each a 2–3 s HTTP call — plus rate-limit errors.
             FIX: Token lookup now reads directly from session_state dict
             (pre-loaded once). get_data_kite() accepts tok_map as param.

  🐛 BUG 3 — fetch_parallel_kite() passed only key+token to workers.
             Workers had no access to the instrument token map (session_state
             is not thread-safe across ThreadPoolExecutor threads).
             FIX: tok_map snapshot passed explicitly into every worker.

  🐛 BUG 4 — st.cache_data on get_data_kite clashes with the unhashable
             KiteConnect object created inside. Also the _kite_key /
             _access_token underscore trick only works for objects passed
             as args, not objects created inside the function.
             FIX: @st.cache_data removed from get_data_kite; caching is
             handled by a plain dict in session_state (data_cache_store).

  🐛 BUG 5 — Kite historical_data() for "day" interval with 730-day
             lookback exceeds the API's allowed window (max ~2000 candles
             per call for equities, but date range cap is ~400 days for
             daily on many instrument types). Also, weekends inflate the
             calendar-day count.
             FIX: Lookback reduced to 400 days for "day", split into two
             calls if needed. For intraday kept within Kite limits.

  🐛 BUG 6 — Silent bare `except: return None` in scan_one() swallowed
             every exception including pandas shape errors after the
             Kite data rename. Added debug logging to st.session_state.

  🐛 BUG 7 — `results or reuse` check: if scan just ran and returned []
             (empty list after filtering), `st.rerun()` loops endlessly
             and the "No signals" warning is never visible because the
             outer `if results or reuse:` is False for [].
             FIX: track scan_ran flag separately from results list.

  🐛 BUG 8 — `enabled_keys` list built inside sidebar `with` block, but
             read outside it during scan. In some Streamlit execution
             orders the list was [] when scan ran.
             FIX: persist enabled_keys to session_state.
"""
